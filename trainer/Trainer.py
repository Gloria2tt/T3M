...
class Trainer():
    def __init__(self) -> None:
        ...
        self.interation = len(self.train_loader)

        self.generator = init_model(self.config.Model.model_name, self.args, self.config,self.args.bert_config,self.interation)
        
        self.optimizer = self.get_optimizer(self.generator.generator)
        
        #self.scheduler = self.get_scheduler(self.optimizer,self.interation,self.interation*self.config.Train.epochs)
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, self.interation*self.config.Train.epochs,eta_min=2e-6 )
        self.start_epoch = 0
        self.global_steps = 0
        if self.args.resume:
            self.resume()
        print('iteration:',self.interation)
        # self.init_optimizer()
    ...
    def print_func(self, loss_dict, steps):
        ...
        info_str += ['%s:%.6f'%(key, loss_dict[key]/steps) for key in list(loss_dict.keys())]
    ...
    def train_epoch(self, epoch):
        ...
        if 'freeMo' in self.config.Model.model_name:
            ...
            for bat in tqdm(zip(self.trans_loader, self.zero_loader)):
                self.global_steps += 1
                epoch_steps += 1
                loss,_, loss_dict = self.generator(bat)
            ...
            for bat in self.train_loader:
                ...
                if self.global_steps % self.config.Log.print_every == 0:
                    self.print_func(epoch_loss_dict, epoch_steps)
            self.loss_nums.append(loss_dict["ce_loss"])
