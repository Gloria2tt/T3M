...
def infer(g_body, g_face, smplx_model, rendertool, config, args,text):
    ...
    for i in range(num_sample):
        ...
        print("pred_pose:",pred_res.shape)
        ...
        if config.Model.model_name == 's2g_LS3DCG':
            ...
            print("pred_jaw: ",pred_jaw.shape)###3 
            print("pred_face: ",pred_face.shape)### 100
            print("pred1:",pred.shape) ### 129
            pred = torch.cat([pred_jaw, pred, pred_face], dim=-1)### jaw:300x3 face:300x100 ### 232
        print("pred__:",pred.shape)
        # pred[:, 9:12] = global_orient
        pred = part2full(pred, stand)
        print("pred2:",pred.shape)
        if face:
            print("body_static:",body_static.shape)
            pred = torch.cat([pred[:, :3], body_static.repeat(pred.shape[0], 1), pred[:, -100:]], dim=-1)
        print("pred3:", pred.shape) #### pred 300x265
    ...
    for i in result_list:
        print("i:",i.shape)
        for j in range(i.shape[0]):
            print("pose",i[j][12:75].shape)
            print("hand:",i[j][75:165].shape)
            print("face:",i[j][165:265].shape)
            break
        break
    ...
    file_name = 'visualise/video_cvpr/' + config.Log.name + '/' + \
...
def main(text):
    ...
    iteration = 0
    print('init model...')
    generator = init_model(body_model_name, body_model_path, args, config,bert_config,iteration)
    generator2 = None
    generator_face = init_model(face_model_name, face_model_path, args, config,bert_config,iteration)
    ...
    rendertool = RenderTool('visualise/eccv/' + config.Log.name)
    infer(generator, generator_face, smplx_model, rendertool, config, args,text)

if __name__ == '__main__':
    
    main(text='pengwenshuo')
