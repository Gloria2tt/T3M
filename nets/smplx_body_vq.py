...
class TrainWrapper(TrainWrapperBaseClass):
    ...
    def init_params(self):
        ...
        self.each_dim = [jaw_dim, eye_dim + body_dim, hand_dim, face_dim] ### 0,39,90,100
    ...
    def __call__(self, bat):
        ...
        poses = poses[:, self.c_index, :] ### mfcc
        print("poses:", poses.shape)
        gt_poses = poses.permute(0, 2, 1)
        b_poses = gt_poses[..., :self.each_dim[1]] ### :39
        h_poses = gt_poses[..., self.each_dim[1]:] #### 39:129

        print("b_pose:",b_poses.shape)
        print("h_poses: ",h_poses.shape)
        if self.composition:
            loss = 0
