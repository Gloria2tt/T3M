...
class qformer_bert(nn.Module):
    def __init__(self,                 
        ...
        self.cls_id = tokenizer._convert_token_to_id('[CLS]')
    ...
    def forward(self,audio_feat,video_input,ablation=False,max_lenth = 150):
        if not ablation:
            ...
            #print("1",audio_token.shape)
            inputs_embeds = self.audioencoder(audio_feat.transpose(1,2)).transpose(1,2)###
            ...
            #inputs_token = torch.cat([cls_token,audio_token],dim=-1)
            ...
            out_put = self.text_encoder(inputs_embeds=audio_embeds,encoder_hidden_states=video_embeds)### bxseq+1x512
            out_put = out_put.last_hidden_state[:,:,]
