...
class qformer(nn.Module):
    def __init__(self,                 
        ...
        self.width = width
        self.proj = nn.Linear(1024,codebook_size,bias=False)
    ...
    def forward(self,audio_token,video_input,motion_token,ablation=False,max_lenth = 150,use_q=False):
        if not ablation:
            ...
            audio_token = torch.cat([audio_token,zeros],dim=1)#### bx(1+tokne)
            input_token = torch.cat([audio_token,motion_token])
            input_embeds = self.embedding(audio_token)
            if len(video_input.shape) == 2:
                video_input = video_input.unsqueeze(0)
            #audio_embeds = input_embeds
            
            audio_embeds = self.position(input_embeds)
