...
class qformer_gpt(nn.Module):
    def __init__(self,                 
                 med_config = '/mnt/nj-aigc/usr/pengwenshuo/TalkSHOW/config/bert_config.json', 
                 batchsize = 128 ,
                 q_lenth = 44,
                 width = 768, 
                 embed_dim = 512,     
        ...
        #self.text_encoder = BertModel.from_pretrained('/mnt/nj-aigc/usr/pengwenshuo/bert-medium',config=encoder_config)
        ...
        #tokenizer = BertTokenizer.from_pretrained('/mnt/nj-aigc/usr/pengwenshuo/bert-medium')
        self.text_encoder = nn.TransformerDecoder(decoder_layer=self.decoder_layer,num_layers=num_layers)
        self.embedding = nn.Embedding(4085, embed_dim)
        self.mlp1 = nn.Linear(128,embed_dim)
        self.cls_id = torch.tensor([4084]).to('cuda')
        ...
        #self.audioencoder = AudioEncoder(in_dim=512,num_hiddens=1024,num_residual_layers=2,num_residual_hiddens=0)
        #self.width = width
        self.proj = nn.Linear(512,codebook_size+1048)
    ...
    def forward(self,audio_token,video_input,motion_token,ablation=False,max_lenth = 150):
        if not ablation:
            #b,seq_len = audio_feat.shape[0],audio_feat.shape[1] #### bx88
            b,seq_len = audio_token.shape[0],audio_token.shape[1]
            #print(audio_token.shape)
            #cls_embedding = self.embedding(self.cls_id).unsqueeze(0).repeat(b,1,1).cuda()
            #audio_embedding = self.mlp1(audio_feat)
            #print("aud",audio_embedding.shape)
            #motion_embedding = self.embedding(motion_token)
            cls_token = torch.tensor(self.cls_id).unsqueeze(0).repeat(b,1).to('cuda')
            #print(cls_token.shape)
            audio_token = audio_token.squeeze(-1)
            inputs_token = torch.cat([audio_token,cls_token],dim=-1)
            #cls_index = audio_embedding.shape[1]
            cls_index = seq_len
            inputs_token = torch.cat([inputs_token,motion_token],dim=-1)### 128x44
            input_embedding = self.embedding(inputs_token)
            #input_embedding = torch.cat([audio_embedding,cls_embedding,motion_embedding],dim=1)
            input_embedding = self.position(input_embedding)
            if len(video_input.shape) == 2:
                video_input = video_input.unsqueeze(0)
            video_embeds = video_input.repeat(1,1,1)
            tgt_mask = self.get_tgt_mask(input_embedding.shape[1], input_embedding.device)
            out_put = self.text_encoder(input_embedding,video_embeds,tgt_mask = tgt_mask)### bxseq+1x512
            #print(out_put.shape)
            out_put = out_put[:,cls_index:-1,]
            #print(out_put.shape)
            #out_put = self.audioencoder(out_put.transpose(1,2)).transpose(1,2) ### bxseqx1024
            projhead = self.proj(out_put)
            #print(projhead.shape)
            try:
                prjhead = projhead.view(b,-1,2,2048+1048)
            except:
                prjhead = projhead[:,:-1,:].view(b,-1,2,2048+1048)
        else:
