...
class SmplxDataset():
    ...
    def __init__(self,
                 ...
                 num_frames=25,
                 num_pre_frames=25,
                 num_generate_length=25, ### 300/4 150 75 body 75hand 300 150 10 300 cnn 1:2 150x768 150 75x2
                 context_info=False, ## 300 88 88 /4 cnn stide 
                 convert_to_6d=False,
                 expression=False,
                 config=None,
                 am=None,
        ...
        if load_mode == 'pickle':
            ...
            if self.load_all:
                self._load_them_all(am, am_sr, motion_fn,video_fn)
    ...
    def _load_them_all(self, am, am_sr, motion_fn,video_fn):
        self.loaded_data = {}
        self.complete_data = []
        f = open(motion_fn, 'rb+')
        data = pickle.load(f)
        video_input = np.load(video_fn)
        self.video_input = torch.from_numpy(video_input)
    ...
    def get_dataset(self, normalization=False, normalize_stats=None, split='train'):
        class __Worker__(data.Dataset):
            ...
            def __getitem__(child, index):
                    ...
                    if not self.whole_video:
                        ...
                        if audio_feat.shape[0] < seq_len:
                            audio_feat = np.pad(audio_feat, [[0, seq_len - audio_feat.shape[0]], [0, 0]],
                                                mode='reflect')
                        #print("yes")
                        """if audio_token.shape[0]< seq_len:
                            audio_token = audio_token[-seq_len:]
                            assert audio_token.shape"""
                        
                        #print(audio_feat.shape,seq_len,self.audio_feat_dim)
                ...
                #print("video_input:",self.video_input.shape)
                #print("aud_feat:",audio_feat.shape)
                #print("aud_token",audio_token.shape)
